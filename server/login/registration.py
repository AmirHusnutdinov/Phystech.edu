...
from server.login.forms import (
    RegistrationForm,
    ConfirmationForm,
)  # Импортируйте новую форму
    ...
    def process_confirmation():
        ...
        if form.validate_on_submit():
            ...
            if str(entered_code) == str(session.get("code")):
                ...
                session["login"] = True
                database_query(
                    f"""INSERT INTO users (login, name, email, password) 
                                VALUES ('{session["name"]}', '{session["name"]}', '{session["email"]}', '{session["password"]}');
                                """,
                    fetch=False,
                )
                session["code"] = None
                ...
                session["email"] = None
                return jsonify(
                    {"success": True, "redirect_url": url_for("open_main_page")}
                )
            else:
                # flash("Invalid confirmation code. Please try again.", "danger")
                return redirect(
                    url_for("confirm_code")
                )  # Перенаправляем обратно на страницу подтверждения
